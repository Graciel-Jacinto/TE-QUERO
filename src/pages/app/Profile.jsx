...
export default function Profile() {
  ...
  /* ---------- Estado local (cópia do perfil para edição) ---------- */
  ...
  /* ---------- Carregar dados quando o perfil chega ---------- */
  useEffect(() => {
  ...
  const pickPhotos = (e) => {
    ...
    const available = MAX_PHOTOS - form.photos.length - photoFiles.length;
    if (available <= 0) {
      return showToast(`Máximo de ${MAX_PHOTOS} fotos.`, 'error');
    }
  ...
  const pickVideo = async (e) => {
    ...
    if (form.videos.length >= MAX_VIDEOS) {
      return showToast(`Máximo de ${MAX_VIDEOS} vídeos.`, 'error');
    }
    if (file.size > MAX_FILE_MB * 1024 * 1024) {
    ...
    <>
      <div className="max-w-[1100px] mx-auto">
        ...
        <div className="bg-white rounded-3xl border border-gray-100 p-6 sm:p-8">
          ...
          {/* ============ FORMULÁRIO COMPLETO (modo edição) ============ */}
          {editing && (
          ...
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-[12.5px] text-gray-500 mt-0.5">
                {totalPhotos} de {MAX_PHOTOS} fotos
              </p>
            </div>

            {editing && totalPhotos < MAX_PHOTOS && (
        ...
        <div className="mt-6 bg-white rounded-3xl border border-gray-100 p-6 sm:p-8">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-[12.5px] text-gray-500 mt-0.5">
                {form.videos.length} de {MAX_VIDEOS} vídeos
              </p>
            </div>

            {editing && form.videos.length < MAX_VIDEOS && (
              <label className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl
                bg-brand-50 text-brand-700 text-[13px] font-semibold
                cursor-pointer hover:bg-brand-100 transition">
                <i className="fi fi-rr-plus text-base leading-none" />
            ...
            <div className="py-10 text-center">
              ...
              {editing && (
                <p className="text-[11.5px] text-gray-400 mt-1">
                  Máximo {MAX_VIDEOS} vídeos · {MAX_FILE_MB}MB cada
                </p>
              )}
            </div>
          ) : (
